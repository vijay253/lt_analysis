...
void single_setting(TString q2_set){
  ...
  vector<Float_t> prv_par_vec, g_vec, w_vec, q2_vec, th_vec, logq2_vec, t_vec;
	...
	para_file_in.open("/u/group/c-kaonlt/USERS/vijay/simc_gfortran/par.pl", ifstream::in);
		//      para_file_in.open("../simc_gfortran/par.pl", ifstream::in);
		//      para_file_in.open("../simc_gfortran/old_parameters/start_par/par.pl", ifstream::in);
		...
		//      para_file_in.open("../parameters/it3/par.pl", ifstream::in);
		//	para_file_in.open("../parameters/it4/par.pl", ifstream::in);
		//	para_file_in.open("../parameters/it5/par.pl", ifstream::in);
...
//       Float_t g       = (2.47**2-m_p**2)**2/(w**2-m_p**2)**2;
        	...
        	Float_t g = pow(pow(2.2002,2)-pow(m_p,2),2)/pow(pow(w,2)-pow(m_p,2),2);
...
//	f_sigT_pre->SetParameters(prv_par_vec[0], prv_par_vec[1], prv_par_vec[2]);
	...
	if (q2_set == "375") {
		...
		f_sigT = new TF1("sig_T", fun_Sig_T_160, 0.00595, 0.060, 2); 
		//f_sigT = new TF1("sig_T", fun_Sig_T_160, 0.00888, 0.04060, 2); 
		...
		f_sigT_pre = new TF1("sig_T_pre", fun_Sig_T_245, 0, 0.5, 4);
		f_sigT_pre->SetParameters(t0, t1, t2, t3);
...
//		q2_term = t2 * q2_vec[i] + t3 * g_sigt->GetX()[i] * q2_vec[i];
		...
		sigt_X_fit     = (g_sigt->GetY()[i])*(pow(q2_vec[i], 2));//  * 2*pi*pow(10,6) * g_vec[i]);
		sigt_X_fit_err = (g_sigt->GetEY()[i])*(pow(q2_vec[i], 2));// * 2*pi*pow(10,6) * g_vec[i]);
...
//	exit(0);
	...
	TFitResultPtr fit_t_result	= g_sigt_fit->Fit(f_sigT, "SR");
...
//		cout << w_vec[i] << "  " << n1->GetV2()[i]<< "  " << f_sigT_pre->Eval(n1->GetV2()[i]) * g_vec[i] << endl;
		...
		sigl_X_fit     = (g_sigl->GetY()[i])*(pow(q2_vec[i], 2));// * 2*pi*pow(10,6) * g_vec[i]);
		//sigl_X_fit     = (g_sigl->GetY()[i]);// * 2*pi*pow(10,6) * g_vec[i]);
	
		sigl_X_fit_err = (g_sigl->GetEY()[i])*(pow(q2_vec[i], 2));// *2*pi*pow(10,6) * g_vec[i]) ;
...
//	f_sigL->SetParameters(prv_par_vec[3], prv_par_vec[4], prv_par_vec[5]);
	...
	TF1* f_sigL = new TF1("sig_L", fun_Sig_L, 0.00595, 0.060, 2);
//	TF1* f_sigL = new TF1("sig_L", fun_Sig_L, 0.00888, 0.04060, 2);
	f_sigL->SetParameters(l0, l1);

	//	f_sigL->SetParLimits(0, 0, 50);
	// 	f_sigL->SetParLimits(1, 0, 40);
	//	f_sigL->SetParLimits(0, 0, 50);
		//	f_sigL->SetParLimits(1, -20, 50);
		//		f_sigL->SetParLimits(2, -20, 50);
		//		f_sigL->SetParLimits(3, 0, 100);
	/*
	f_sigL->SetParLimits(0, -100, 100);
...
//	g_sigl_fit->Fit(f_sigL);
	TFitResultPtr fit_l_result	= g_sigl_fit->Fit(f_sigL, "SR");
...
//	ROOT::Math::MinimizerOptions::SetDefaultMaxFunctionCalls(100000); 
	TF1* f_sigLT = new TF1("sig_LT", fun_Sig_LT, 0.00595, 0.060, 3); 
//	TF1* f_sigLT = new TF1("sig_LT", fun_Sig_LT, 0.00888, 0.04060, 3); 
	f_sigLT->SetParameters(lt0, lt1, lt2);

	//f_sigLT->SetParLimits(0, -1, 1);
	//	f_sigLT->SetParLimits(1, -1000,  1000);
	//	f_sigLT->SetParLimits(2, -100,  100);

	TFitResultPtr fit_lt_result = 	g_siglt_fit->Fit(f_sigLT, "SR");
...
//	f_sigTT->SetParameters(prv_par_vec[9], prv_par_vec[10], prv_par_vec[11]);
	...
	TF1* f_sigTT = new TF1("sig_TT", fun_Sig_TT, 0.00595, 0.060, 3); 
//	TF1* f_sigTT = new TF1("sig_TT", fun_Sig_TT, 0.00888, 0.04060, 3); 
	...
	*/
	TFitResultPtr fit_tt_result = g_sigtt_fit->Fit(f_sigTT, "SR");
...
Double_t fun_Sig_LT(Double_t *x, Double_t *par) {
   ...
   //Double_t f = (par[0]*exp(par[1]*(xx))+par[2]/(xx));
   //   Double_t f = (par[0]*exp(par[1]*(xx))+par[2]);
   Double_t f = (par[0]/pow((xx),2)*exp(par[1]/(xx))+par[2]/pow((xx),1));
...
Double_t fun_Sig_TT(Double_t *x, Double_t *par) {
   ...
   //   Double_t f = ((par[0]/pow((xx),3)*exp(par[1]*(xx))+par[2]/pow((xx),1)))*(xx/pow((xx+pow(0.139570, 2)), 2));
   Double_t f = ((par[0]/pow((xx),3)*exp(par[1]*(xx))+par[2]/pow((xx),1)));
