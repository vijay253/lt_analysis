...
//	f_sigT_pre->SetParameters(prv_par_vec[0], prv_par_vec[1], prv_par_vec[2]);
	...
	if (q2_set == "375") {
		...
		f_sigT = new TF1("sig_T", fun_Sig_T_160, 0.0, 0.05, 2); 
...
//	f_sigL->SetParameters(prv_par_vec[3], prv_par_vec[4], prv_par_vec[5]);
	...
	TF1* f_sigL = new TF1("sig_L", fun_Sig_L, 0.00, 0.05, 2);
...
*/
	TF1* f_sigLT = new TF1("sig_LT", fun_Sig_LT, 0.00, 0.05, 3); 
...
//	f_sigTT->SetParameters(prv_par_vec[9], prv_par_vec[10], prv_par_vec[11]);
	...
	TF1* f_sigTT = new TF1("sig_TT", fun_Sig_TT, 0.00, 0.05, 3); 
