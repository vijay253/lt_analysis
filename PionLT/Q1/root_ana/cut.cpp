...
TString Missingmass_cut(Double_t m_m_offset) {
	...
	-------------------------------------------------------------------*/
	cut_tmp.Form("(MMp-%f) > 0.875  && (MMp-%f) < 0.98", m_m_offset, m_m_offset);
...
TString Set_t_limit(Double_t tmin, Double_t tmax) {
	...
	-------------------------------------------------------------------*/
	cut_tmp.Form("MandelT > %f && MandelT < %f", tmin, tmax);
...
TString Cointime_primary_cut(Double_t center) {
	...
	if(center > 0) {
		...
		cut_tmp.Form("-1 < (CTime_epCoinTime_ROC1 - %f) && (CTime_epCoinTime_ROC1 - %f) < 1", center, center);
...
TString Cointime_random_cut(Double_t center) {
	...
	if(center > 0) {
		...
		cut_tmp.Form("1 < (CTime_epCoinTime_ROC1 - %f) && (CTime_epCoinTime_ROC1 - %f) < 7", center, center);
...
TString Cointime_all(Double_t center) {
	...
	if(center > 0) {
		...
		cut_tmp.Form("-20 < (CTime_epCoinTime_ROC1 - %f) && (CTime_epCoinTime_ROC1 - %f) < 10", center, center);
