...
c      open(88,file='simc_gfortran/par.pl',status='old')      
      open(88,file='/u/group/c-kaonlt/USERS/vijay/simc_gfortran/par.pl'
     *     ,status='old')      
      do while(.true.)
...
c      sigL = (fitpar(5)*exp(fitpar(6)*(tp)))/(q2**2)
c      sigL = (fitpar(5)*exp(fitpar(6)*(tp)))
c      sigL=(fitpar(5)*exp(fitpar(6)*(tp))+fitpar(7)*exp(fitpar(8)*(tp)))
      sigL=((fitpar(5)*exp(fitpar(6)*(tp)))
     1     * (tp/((-tp - 0.13957*0.13957)**2 * (0.44*0.44 - tp)**2)))
     1     *(1-0.0)
c      sigL = (fitpar(5)*exp(fitpar(6)*(tp))
c     1     *(1-fitpar(7)*tp*exp(fitpar(8)*tp))**4)/(q2**2)
c      sigL = (fitpar(5)*exp(fitpar(6)*(tp))
c     1     *(1-fitpar(7)*tp*exp(fitpar(8)*tp))**4)

c      sigL = ((fitpar(5) + fitpar(6) * log(q2**2))
c     1     * exp((fitpar(7) + fitpar(8) * log(q2**2))*(tp -0.2)))     

c      sigL = ((fitpar(5) + fitpar(6) *(q2))
c     1     * exp((fitpar(7) + fitpar(8) * (q2))*(tp -0.2)))     

c      sigL = ((fitpar(5) + fitpar(6) *(tp))
c     1     * exp((fitpar(7) + fitpar(8) * (tp))*(tp -0.2)))     
...
c test
      ...
      ftav = (abs(tp) - tav)/tav
c      sigT = (fitpar(1)+fitpar(2)*ftav)/(q2**2)
c      sigT = (fitpar(1)+fitpar(2)*ftav)
      sigT = (fitpar(1)+fitpar(2)*ftav**1+fitpar(3)*ftav**2)*(1-0.0)
c      sigT = (fitpar(1)+fitpar(2)*exp(fitpar(3)*-ftav)
c     1    +fitpar(4)*exp(-ftav**2) )/(q2**2)
c      sigT = (fitpar(1)+fitpar(2)*exp(fitpar(3)*(-ftav))
c     1    +fitpar(4)*exp(-ftav**2))

c      sigT = (fitpar(1) + fitpar(2) * ftav
c     1    + (fitpar(3) + fitpar(4) * ftav) * ftav)

c      sigLT = ((fitpar(9)*exp(fitpar(10)*(tp))+fitpar(11)/(tp))
c     1     *sin(thetacm))
c      sigLT = ((fitpar(9)*exp(fitpar(10)*(tp))+fitpar(11))
c     1     *sin(thetacm))
      sigLT = ((fitpar(9)/tp**2*exp(fitpar(10)/(tp))+fitpar(11)/tp)
     1     *sin(thetacm))
...
c      sigTT = ((fitpar(12)/q2*exp(-q2))*ft*sin(thetacm)**2)
c      sigTT = (((fitpar(12)*exp(fitpar(13)*(tp))+fitpar(14))/q2
c     1     *exp(-q2))*ft*sin(thetacm)**2)
      sigTT = (((fitpar(12)/tp**2*exp(fitpar(13)*(tp))
     1     +fitpar(14)/tp**4))*sin(thetacm)**2)
