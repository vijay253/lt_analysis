...
def fill_histogram(input_file, output_file, tree_name, var_name, cut_expression, offset_variable, offset_values, bins, range_min, range_max):
    ...
    with uproot.open(input_file) as file:
        ...
        #if "CTime_ROC1" in data and "CTime_ROC1" in data:
        mask = (data["CTime_ROC1"] > 46) & (data["CTime_ROC1"] < 48)
      
        #for key in data:
            #if key != offset_variable:
            #data[key] = data[key]+offset_values
        
        #mask = (data["CTime_ROC1"] > 46) & (data["CTime_ROC1"] < 48)
        #else:
            #raise ValueError("Branches 'x'")

        filtered_data = {key: array[mask] for key, array in data.items()}
      #  hist = ROOT.TH1F("hist", " ", 300, -50, 50)
        
        #  hist, edges = np.histogram(filtered_data[var_name], bins=bins, range=(range_min, range_max))
       # hist.fill(filtered_data[var_name])
       # tree.Draw(f"{filtered_data[var_name]}>>hist")
