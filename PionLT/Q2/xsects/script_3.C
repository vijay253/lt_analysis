...
Double_t SL(Double_t *x, Double_t *par);
...
//	f_sigT_pre->SetParameters(prv_par_vec[0], prv_par_vec[1], prv_par_vec[2]);
	...
	if (q2_set == "425") {
		...
		f_sigT = new TF1("sig_T", fun_Sig_T_160, 0.00705, 0.060, 2); 
...
//	exit(0);
	...
	TFitResultPtr fit_t_result	= g_sigt_fit->Fit(f_sigT, "ESR");
...
//  	TF1* f_sigL = new TF1("sig_L", fun_Sig_L, 0.00, 0.065, 2);
  	//TF1* f_sigL = new TF1("sig_L", fun_Sig_L, 0.00705, 0.06315, 2);

	TF1* f_sigL = new TF1("sig_L", fun_Sig_L, 0.00705, 0.060, 2);
	...
	TFitResultPtr fit_l_result	= g_sigl_fit->Fit(f_sigL, "ESR");
...
//	Double_t g_min = TMath::MinElement(g_siglt->GetN(), g_siglt->GetY());
	...
	TF1* f_sigLT = new TF1("sig_LT", fun_Sig_LT, 0.00705, 0.060, 3); 

	f_sigLT->SetParameters(lt0, lt1, lt2);

	TFitResultPtr fit_lt_result = 	g_siglt_fit->Fit(f_sigLT, "ESR");
...
//	f_sigTT_pre->SetParameters(prv_par_vec[9], prv_par_vec[10], prv_par_vec[11]);
	...
	TF1* f_sigTT_pre = new TF1("sig_TT", fun_Sig_TT, 0, 0.060, 1);
...
//	Double_t g_min = TMath::MinElement(g_sigtt->GetN(), g_sigtt->GetY());
	...
	TF1* f_sigTT = new TF1("sig_TT", fun_Sig_TT, 0.00705,  0.065, 3); 

	//f_sigTT->SetParameter(0, tt0);
	f_sigTT->SetParameters(tt0, tt1, tt2);

	TFitResultPtr fit_tt_result = g_sigtt_fit->Fit(f_sigTT, "ESR");
...
Double_t fun_Sig_TT(Double_t *x, Double_t *par) {
   ...
   Double_t f = ((par[0]/pow((xx),3)*exp(par[1]*(xx))+par[2]/pow((xx),4)));
