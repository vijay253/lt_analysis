...
void single_setting(TString q2_set){
  ...
  Float_t eps_lo_setting, eps_mi_setting, eps_hi_setting;
	...
	if (q2_set == "425" ) {
		...
		eps_lo_setting = 0.264;
		eps_mi_setting = 0.617;
		eps_hi_setting = 0.774;
...
//  	 	c1->Update();
 ...
 //		TF2 *fff2 = new TF2("fff2","[0] + y*[1]", 0, 360, 0, 0.6);
		...
		//		TF2 *fff2 = new TF2("fff2","[0] + y*[1] + sqrt(2*y*(1+y))*[2]*cos(x*pi/180)  + y*[3]*cos(2*x*pi/180)", 11.25, 348.75, 0.26, 0.80 );       // VK

		//TF2 *fff2 = new TF2("fff2","[0] + y*[1] + sqrt(2*y*(1+y))*[2]*cos(x*pi/180)  + y*[3]*cos(2*x*pi/180)", 11.25, 348.75, 0.238, 0.80);       // Feb 20 VK
		//Divided by 2Pi On March 31, 2025
		// Change it iter. done
		TF2 *fff2 = new TF2("fff2","([0] + y*[1] + sqrt(2*y*(1+y))*[2]*cos(x*pi/180)  + y*[3]*cos(2*x*pi/180))/(2*TMath::Pi())", 11.25, 348.75, 0.238, 0.80);       // Feb 20 VK
		//TF2 *fff2 = new TF2("fff2","([0] + y*[1] + sqrt(2*y*(1+y))*[2]*cos(x*pi/180)  + y*[3]*cos(2*x*pi/180))/(2*TMath::Pi())", 11.25, 348.75, 0.2635, 0.7745);       // Feb 20 VK
		...
		//fff2->SetParameter(0, 100);
  		//fff2->SetParLimits(0, 10,100 );
 ...
 // 		f2->FixParameter(1, test_t);
		...
		f3->Draw("same");	
               //========================================================================================================================================                                            
                //VK,April 14, 2026                                                                                                                                                                   
                // =====================================================                                                                                                                              
                // PRL QUALITY SINGLE t-BIN PLOT (ADDED ONLY)                                                                                                                                         
                // =====================================================                                                                                                                              
                ...
                if(i == prl_tbin){
                  ...
                  // Style graphs                                                                                                                                                                                      g1->SetMarkerStyle(20);
...
//      fff2->GetParameter(3)
 ...
 //		exit(0);
	...
	/*
	label.DrawLatex(0.14, 0.96, "-t = 0.010");
	label.DrawLatex(0.64, 0.96, "-t = 0.015");
	label.DrawLatex(0.14, 0.71, "-t = 0.020");
	label.DrawLatex(0.64, 0.71, "-t = 0.025");
	label.DrawLatex(0.14, 0.46, "-t = 0.031");
	label.DrawLatex(0.64, 0.46, "-t = 0.039");
	label.DrawLatex(0.14, 0.21, "-t = 0.054");
	*/
	label.DrawLatex(0.14, 0.96, "-t = 0.011");
	label.DrawLatex(0.64, 0.96, "-t = 0.017");
	label.DrawLatex(0.14, 0.71, "-t = 0.022");
	label.DrawLatex(0.64, 0.71, "-t = 0.029");
	label.DrawLatex(0.14, 0.46, "-t = 0.038");
	label.DrawLatex(0.64, 0.46, "-t = 0.050");
	label.DrawLatex(0.14, 0.21, "-t = 0.058");
	...
	*/
	...
	//VK Aug 31, 2025
	sig_L_g->GetYaxis()->SetRangeUser(4.0,22.0);
	sig_L_g->Draw("a*");
	...
	*/
	...
	//VK Aug 31, 2025
	sig_T_g->GetYaxis()->SetRangeUser(4.0,22.0);
	sig_T_g->Draw("a*");
	...
	*/
	//sig_LT_g->GetYaxis()->SetRangeUser(-0.75,0.60);
	//VK Aug 31, 2025
	sig_LT_g->GetYaxis()->SetRangeUser(-4.0,4.0);
	sig_LT_g->Draw("a*");
	...
	*/
	//sig_TT_g->GetYaxis()->SetRangeUser(-0.75,0.60);
	//VK
	sig_TT_g->GetYaxis()->SetRangeUser(-4.0,4.0);
	sig_TT_g->Draw("a*");
...
//change iter. done
	xs = (par[0] +  eps*par[1] + sqrt(2*eps*(1+eps))*par[2]*cos(xx*pi/180) + eps*par[3]*cos(2*xx*pi/180))/(2*TMath::Pi());
