...
//void Analysed(Double_t RunNum = 0)
{
  ...
  if(Hostname.Contains("farm")){
    ...
    OutPath = "/group/c-kaonlt/USERS/vijay/lt_analysis/LTsep/Analysis/OUTPUT";
  }
  ...
  //##############################################################################
  TH1D *Coin     = new TH1D("Coin","CTime_ePiCoinTime_ROC1; CTime_ePiCoinTime_ROC1;", 300, -100, 100);
  ...
  for(Long64_t i = 0; i < nEntries_TBRANCH; i++)
    {
      ...
      epicointime =  CTime_ePiCoinTime_ROC1 >= -1.0 && CTime_ePiCoinTime_ROC1 <= 1;// && H_cer_npeSum >= 0.0;
      epicointimeRandom  = CTime_ePiCoinTime_ROC1 >= -15.0 && CTime_ePiCoinTime_ROC1 <= -13.0;   //0.6
  ...
  Double_t err = (1.0/In)*(sqrt(InC*(1-InC/In)));
