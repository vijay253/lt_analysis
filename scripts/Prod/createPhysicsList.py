...
if TargetType == "dummy":
    # Open a file in write mode
    with open('{}/src/lists/list.dummy_{}_{:.0f}'.format(LTANAPATH,Q2.replace(".",""),float(EPSVAL)*100), 'a') as f:
        if float(runNumRight[0]) != 0.0:
            # Write the value of the variable to the file
    ...
    # Open a file in write mode
    with open('{}/src/lists/list.{}_{:.0f}'.format(LTANAPATH,Q2.replace(".",""),float(EPSVAL)*100), 'a') as f:
        if float(runNumRight[0]) != 0.0:
            # Write the value of the variable to the file
            for i,thpq in enumerate(EbeamValRight):
                # convert uC to C (10^-6C=1uC)
