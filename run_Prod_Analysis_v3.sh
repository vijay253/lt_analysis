...
#    PATHFILE_INFO=`python3 /u/home/${USER}/.local/lib/python3.4/site-packages/ltsep/scripts/getPathDict.py $PWD` # The output of this python script is just a comma separated string
    PATHFILE_INFO=`python3 /u/group/c-kaonlt/USERS/vijay/replay_lt_env/lib/python3.9/site-packages/ltsep/scripts/getPathDict.py $PWD` # The output of this python script is just a comma separated string
fi
...
if [[ $t_flag = "true" || $a_flag = "true" ]]; then
    ...
    echo "Q2 must be one of - [0p375 - 0p425]"
    echo "W must be one of - [2p2]"
	...
	while true; do
	    echo ""
	    read -p "Epsilon must be - high - mid - low - Case Sensitive! - or press ctrl-c to exit : " EPSILON
	    case $EPSILON in
...
declare -a PHI0P425HE=("CENTER" "LEFT1" "LEFT2")         # Q2 & mid epsilon setting
for i in "${PHI0P425HE[@]}"
do
    if [[ $Q2 = "0p425" && $W = "2p2" && $EPSILON = "mid" ]]; then
...
declare -a PHI0P425HE=("CENTER" "LEFT1" "LEFT2")         # Q2 & low epsilon setting
for i in "${PHI0P425HE[@]}"
do
    if [[ $Q2 = "0p425" && $W = "2p2" && $EPSILON = "low" ]]; then
